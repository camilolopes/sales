...
# app.py
# -*- coding: utf-8 -*-
# v4.9.6 (produção | packages.txt + deps estáveis)
...
APP_VERSION = "v4.9.6 (produção | packages.txt + deps estáveis)"
...
st.set_page_config(page_title="Indicadores Drogaria – v4.9.6 (produção | packages.txt + deps estáveis)", layout="wide")
...
if df is not None:
    ...
    proceed = st.button("Gerar Indicadores (v4.9.6)")
    ...
    if proceed:
        ...
        if missing:
            ...
            st.subheader("📌 Resumo do Período (v4.9.6)")
            ...
            st.divider()
            st.markdown("### ⬇️ Download do Excel (v4.9.6)")
